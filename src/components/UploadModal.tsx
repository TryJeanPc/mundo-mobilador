...
import { X, UploadCloud, User, Lock, Plus, Folder, CheckCircle, ChevronRight, Trash2 } from 'lucide-react';
...
export function UploadModal({ isOpen, onClose, onUpload, authors, onAddAuthor, onDeleteAuthor }: UploadModalProps) {
  ...
  const handleModUpload = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const newMod: ModApk = {
      ...
      category: selectedAuthor.category, // Auto-assign based on author
          ...
          {step === 'MOD_UPLOAD' && selectedAuthor && (
            <form onSubmit={handleModUpload} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                {uploadMethod === 'file' ? (
                  <label className="flex flex-col items-center justify-center border border-dashed border-zinc-700 hover:border-cyan-400 bg-black/50 py-6 cursor-pointer transition-colors text-center group">
                    <UploadCloud className="w-6 h-6 text-zinc-500 group-hover:text-cyan-400 mb-2 transition-colors" />
                    <span className="font-mono text-xs text-zinc-400 group-hover:text-cyan-400">Seleccionar archivo .APK</span>
              ...
              <div className="pt-4 border-t border-zinc-800 flex gap-4">
                <button 
                  type="button" 
                  onClick={() => setStep('AUTHOR_SELECT')}
                  className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 py-3 font-mono text-sm uppercase tracking-wider transition-colors"
                >
                  Atrás
                </button>
                <button 
                  type="submit" 
                  className="flex-[2] bg-cyan-500 hover:bg-cyan-400 text-black py-3 font-mono text-sm uppercase font-bold tracking-wider transition-colors flex justify-center items-center gap-2"
                >
